...
Around the end of January 2020, I enrolled in the HarvardX Data Science for Professionals certificate program. So far, I have completed two of the nine courses for this program. The courses consist of several units per course, with each unit having a collection of videos that correspond with chapters in the textbook. The courses are taught by Professor Rafael Irizarry. 
...
## Setup Steps
All examples will use R. Before you get started, downloading and install R and RStudio on your development machine

https://www.r-project.org/

## My Background & Forst Impressions
Starting this course, I was already familiar with programming in general, and SQL in particular for data operations. I have been doing full stack development in the dotnet/SQL sphere for about 20 years professionally. My first impression, coming from a web programming background, is that R, dotnet, and SQL features overlap substantially. Syntax is similar to javascript or C#, but more fundamentally built around vectors / lists vs individual variable values. ALthough every language can handle arrays and hashtables in some manner - R implements these constructs very elegantly. This makes sense, as statistcs is all about set theory, and this is what R was built for.  Thre is also connectivity between SQL and R via the ODBC package... allowing you to blend the two and choose cases where R offers advantages, or cases where SQL might be better for a specific analysis task. You can always split analysis between your SQL and web logic layers- but R offers a powerful middle layer to create reproducible, discrete code blocks for logical analysis and take data from teh database to a finished shart without having to run through so many transformations. R is especially suited to charting and visualization of data, random sampling, modeling, and statistical analysis tasks.
...
A cector in R is equivalent to a one-dimensional array in javascript or a row in SQL. A vector represents x observations (fields) of the same parameter / column. A basic list of values that can be access through square braces. 
Data frames are extensions of this concept, enabling assignment of tables of name/value pairs where each entry is a vector of values associated with a named column header

Although the course is called R basics - there is also a focus on the science behind data - like how you should think about data - is the data binary? character? numeric? if numeric - are teh values continuous or discrete? Are caharacters categorical factors or ordinal? All of these core principles drive future decisions when analyzing and manipulating your data, choosing models and determining how to wrangle your data sets.
...
basic if/else logic can be applied across vectors using a helpful shortcut:
